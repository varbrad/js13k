...
const files = fs
  ...
  .forEach(file => {
    zip.file(
      file,
      fs.readFileSync(path.join(__dirname, "dist", file), { encoding: "utf-8" })
    );
  });
...
zip
  ...
  .on("finish", () => {
    ...
    if (size <= limit) {
      console.log(
        chalk.green(
          `Only ${percentage}% of space used! Still ${prettySpace} left!`
        )
      );
    } else {
      console.log(
        chalk.red(
          `Uh-oh, you are over the ${prettyLimit} limit! (${percentage}% used!)`
        )
      );
    }
  });
